 ...
 */
export async function signUpDev(email, password) {
    try {
 ...
 */
export async function signInDev(email, password) {
    try {
        ...
        if (!user) {
            ...
            const db = getDb();
            const devUserRow = db.prepare('SELECT du.email, du.password_hash, p.user_id FROM dev_users du JOIN profiles p ON du.email = p.email WHERE du.email = ?').get(email);
            if (!devUserRow) {
 ...
 */
export async function loadUserDataDev(identifier) {
    try {
        const db = getDb();
 ...
 */
export async function verifyAccessTokenDev(token) {
    try {
 ...
 */
export async function requestPasswordResetDev(_email) {
    return { success: true };
}
/**
...
export function initDevMode() {
  ...
  `);
    ...
    const devUserRows = db.prepare('SELECT du.email, du.password_hash, p.user_id FROM dev_users du JOIN profiles p ON du.email = p.email').all();
