...
import { z } from 'zod';
import { addProviderKey, getProviderKeysByUser, getProviderKeyDecrypted, deleteProviderKey, ensureUserUnifiedKey, regenerateUserUnifiedKey, getUserSetting, setUserSetting, getUserUsageSummary, getUserUsageByProvider, } from '../db/supabase-service.js';
...
userDataRouter.post('/keys', async (req, res) => {
    try {
...
userDataRouter.get('/keys', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.get('/keys/:keyId', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.patch('/keys/:keyId', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.delete('/keys/:keyId', async (req, res) => {
    try {
        const userId = getUserId(req);
        const keyId = typeof req.params.keyId === 'string' ? req.params.keyId : req.params.keyId[0];
...
userDataRouter.get('/unified-key', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.post('/unified-key/regenerate', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.get('/settings/:key', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.put('/settings/:key', async (req, res) => {
    try {
        const userId = getUserId(req);
        const key = typeof req.params.key === 'string' ? req.params.key : req.params.key[0];
...
userDataRouter.get('/usage/summary', async (req, res) => {
    try {
        const userId = getUserId(req);
...
userDataRouter.get('/usage/by-provider', async (req, res) => {
    try {
        const userId = getUserId(req);
        const days = parseInt(req.query.days) || 30;
