...
export function logRequest(
  ...
  getPersistence().enqueueWrite(async () => {
    ...
    if (!sb) return;
    let supabaseKeyId: number | null = null;
    if (keyId) {
      const localKey = getDb().prepare(
        'SELECT platform, label FROM api_keys WHERE id = ?',
      ).get(keyId) as { platform: string; label: string } | undefined;
      if (localKey) {
    ...
    if (insErr) throw new Error(`requests insert: ${insErr.message}`);
