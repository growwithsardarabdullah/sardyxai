...
keysRouter.use(requireAuth as any);
...
function maskKey(platform: string, id: number): string {
...
keysRouter.post('/', (req: Request, res: Response) => {
  ...
  const row = db.prepare('SELECT * FROM api_keys WHERE id = ?').get(result.lastInsertRowid) as any;
...
keysRouter.post('/custom', (req: Request, res: Response) => {
  ...
  let keyRow = db.prepare("SELECT * FROM api_keys WHERE platform = 'custom' AND base_url = ?").get(baseUrl) as any;
  if (!keyRow) {
    let enc: ReturnType<typeof encrypt>;
    try {
      enc = encryptKey(key);
    ...
    keyRow = db.prepare('SELECT * FROM api_keys WHERE id = ?').get(result.lastInsertRowid) as any;
  }
  ...
  const modelRow = db.prepare("SELECT id FROM models WHERE platform = 'custom' AND model_id = ?").get(model) as any;

  // Ensure fallback_config entry
  const existing = db.prepare('SELECT id FROM fallback_config WHERE model_db_id = ?').get(modelRow.id);
  if (!existing) {
    const maxPriority = (db.prepare('SELECT COALESCE(MAX(priority), 0) AS mx FROM fallback_config').get() as any).mx;
