...
export async function addProviderKey(
  ...
  if (error) throw new Error(`Failed to add provider key: ${error.message}`);
  return data;
}
...
export async function getProviderKeysByUser(userId: string) {
  ...
  if (error) throw new Error(`Failed to fetch provider keys: ${error.message}`);
  return data || [];
}

export async function getProviderKeyDecrypted(userId: string, keyId: string) {
  ...
  if (error) throw new Error(`Failed to fetch provider key: ${error.message}`);
  if (!data) throw new Error('Provider key not found');
...
export async function updateProviderKeyStatus(keyId: string, status: string) {
  const supabase = getSupabaseClient();
  const { error } = await supabase
    .from('provider_keys')
  ...
  if (error) throw new Error(`Failed to update provider key status: ${error.message}`);
}
...
export async function ensureUserUnifiedKey(userId: string) {
  const supabase = getSupabaseClient();
  ...
  if (error) throw new Error(`Failed to generate unified key: ${error.message}`);
  return data;
}

export async function regenerateUserUnifiedKey(userId: string) {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase
    .rpc('regenerate_unified_key', { p_user_id: userId });
  
  if (error) throw new Error(`Failed to regenerate unified key: ${error.message}`);
  return data;
}
...
export async function getUserUsageByProvider(userId: string, days: number = 30) {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase
    .from('usage_logs')
